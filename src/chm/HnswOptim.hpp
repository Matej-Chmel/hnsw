...
namespace chm {
	...
	class HnswOptim {
		...
		void searchLowerLayer(const Dist* const query, const size_t ef, const size_t lc, FarHeap<Dist>& W);
	...
	inline void HnswOptim<Dist>::insert(const Dist* const query) {
		...
		for(auto lc = std::min(L, l);; lc--) {
			this->searchLowerLayer(query, this->efConstruction, lc, this->farHeap);
	...
	inline FarHeap<Dist> HnswOptim<Dist>::search(const Dist* const query, const size_t K, const size_t ef) {
		...
		this->searchLowerLayer(query, ef, 0, W);
